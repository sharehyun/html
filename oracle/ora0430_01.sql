...
--update stuscore set kor=kor+10, total=kor+eng+math, avg=total/3;
...
--컬럼 총 8개 - sno,name,kor,eng,math,total,avg,rank
select * from stuscore;

select * from stuscore where avg>=80;

-- 평균 80점이상이면서, 국어점수가 90점이상을 검색하시오.
...
update stuscore a
set rank = (select sno,total,rank() over (order by total desc) as ranks from stuscore b
where a.sno=b.sno);
...
select kor+eng ke from stuscore s;
...
select ranks from (select sno,rank() over(order by total desc) ranks from stuscore3 where a.sno=b.sno);
...
select * from employees;
-- 월급 4000이상이면서 부서번호가 50이상인 사원을 검색하시오.

select * from employees where department_id>=50 and salary>=4000 order by employee_id;

-- 사원번호 150번 이상이거나 입사일이 2007/01/01 이전인 사원을 검색하시오.
select count(*) from employees;
select * from employees where employee_id>=150 or hire_date<'07/01/01';
...
-- 이름 중에 n이 들어가 있는 사원을 출력하시오.
select * from employees where emp_name like'%n%';

-- 이름 2번째 철자에 o가 들어가 있는 사원을 출력하시오.
...
-- 이름에 '순'이 들어가는 학생을 검색하시오.
select name from stuscore where name like '%순%';

-- 2번째 a
select name from stuscore where name like '__a%';

select emp_name from employees;
select emp_name,job_id from employees where job_id like '%#_%' escape '#';

-- 입사일이 00/01/01~07/12/31
select hire_date from employees where hire_date>='00/01/01' and hire_date>='07/12/31';
select hire_date from employees where hire_date between '00/01/01' and '07/12/31' order by hire_date;



--member 테이블에서 날짜가 2025년 이후에 가입한 회원을 출력하시오.
...
-- salary, 연봉, 1438원, 한화 천단위 표시 -> 3번째 
select emp_name,salary, salary*12, salary+salary*nvl(commission_pct,0), -- 커미션 추가된 월급
salary*12*1438,to_char(salary*12*1438, '999,999,999') from employees where lower(emp_name) like '__g%';
...
select email from employees where email like '%E%' or email like '%S%' order by email asc;
select email from employees where email like '%E%' and email like '%S%' order by email desc;

select * from stuscore;

-- 2개 order by 정렬, 1차 우선 정렬 후, 같은 값일 경우 2차 정렬 진행
